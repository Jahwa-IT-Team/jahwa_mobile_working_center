...
class JimsMESUnLock extends StatefulWidget {
  @override
  _JimsMESUnLockState createState() => _JimsMESUnLockState();
}

class _JimsMESUnLockState extends State<JimsMESUnLock> {
  ...
  void initState() {
    super.initState();
    print("open MES UnLock Page : " + DateTime.now().toString());
  }
    ...
    return GestureDetector( /// Keyboard UnFocus시를 위해 onTap에 GestureDetector를 위치시킴
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text('MES UnLock', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white, fontSize: 15,),),
        ),
            ...
            jsonDecode(response.body)['Table'].forEach((element) {
              ...
              AlertDialog alert = AlertDialog(
                ...
                actions: <Widget>[
                  FlatButton(
                    onPressed: () {
                      unlockMES(context, element['EmpCode']);
  ...
  /// Show Message Box Function
  unlockMES(BuildContext context, String code) async {
    ...
    try {
      ...
      // Login API Url
      var url = 'https://jhapi.jahwa.co.kr/JimsMESUnLock';
      ...
      return await http.post(Uri.parse(url), body: json.encode(data), headers: {"Content-Type": "application/json"}).timeout(const Duration(seconds: 15)).then<bool>((http.Response response) async {
        if(response.statusCode == 400) { showMessageBox(context, 'Alert', response.body); }
        else if(response.statusCode != 200 || response.body == null || response.body == "{}" ){ showMessageBox(context, 'Alert', 'MES UnLock Error !!!'); }
        else if(response.statusCode == 200){
          if(jsonDecode(response.body)['Table'].length == 0) {
            showMessageBox(context, 'Alert', 'Data does not Exists!!!');
          }
      ...
      showMessageBox(context, 'Alert', 'MES UnLock Error : ' + e.toString());
    }
  }
}
